...
class DataSyncService {
     ...
     */
    exportFullSync(zaloIds, employeeId) {
        ...
        const accounts = db.query(`SELECT ${ACCOUNT_SAFE_COLUMNS} FROM accounts WHERE zalo_id IN (${placeholders})`, zaloIds);
        ...
        for (const spec of SYNCABLE_TABLES_BY_ZALO) {
            try {
                ...
                let allRows = [];
                while (true) {
                    const batch = db.query(`SELECT * FROM ${spec.table} WHERE ${spec.zaloCol} IN (${placeholders}) LIMIT ${PAGE_SIZE} OFFSET ${offset}`, zaloIds);
                    if (batch.length === 0)
                        break;
                ...
                Logger_1.default.warn(`[DataSyncService] Export skip ${spec.table}: ${err.message}`);
            }
        }
        // Export global tables (paginated)
        for (const spec of SYNCABLE_TABLES_GLOBAL) {
            if (employeeId && PRIVACY_FILTERED_ERP_TABLES.has(spec.table))
                continue;
            try {
                let offset = 0;
                let allRows = [];
                while (true) {
                    const batch = db.query(`SELECT * FROM ${spec.table} LIMIT ${PAGE_SIZE} OFFSET ${offset}`);
                    if (batch.length === 0)
                        break;
                    allRows = allRows.concat(batch);
                    if (batch.length < PAGE_SIZE)
        ...
        Logger_1.default.log(`[DataSyncService] Full export: ${Object.keys(tables).length} tables, ${totalRows} rows for ${zaloIds.length} accounts`);
     ...
     */
    exportDeltaSync(zaloIds, sinceTs, employeeId) {
        const db = DatabaseService_1.default.getInstance();
        const syncTs = Date.now();
        const tables = {};
        const placeholders = zaloIds.map(() => '?').join(',');
        ...
        for (const spec of SYNCABLE_TABLES_BY_ZALO) {
            try {
                let rows;
                if (spec.tsCol) {
                    rows = db.query(`SELECT * FROM ${spec.table} WHERE ${spec.zaloCol} IN (${placeholders}) AND ${spec.tsCol} > ?`, [...zaloIds, sinceTs]);
                }
                else {
                    // No timestamp column — full re-export for this table
                    rows = db.query(`SELECT * FROM ${spec.table} WHERE ${spec.zaloCol} IN (${placeholders})`, zaloIds);
                }
                if (rows.length > 0) {
                ...
                Logger_1.default.warn(`[DataSyncService] Delta skip ${spec.table}: ${err.message}`);
            }
        }
        ...
        for (const spec of SYNCABLE_TABLES_GLOBAL) {
            ...
            try {
                let rows;
                if (spec.tsCol) {
                    rows = db.query(`SELECT * FROM ${spec.table} WHERE ${spec.tsCol} > ?`, [sinceTs]);
                }
                else {
                    rows = db.query(`SELECT * FROM ${spec.table}`);
                }
                if (rows.length > 0) {
                    tables[spec.table] = rows;
                }
        ...
        if (employeeId)
            this.appendPrivateErpTables(tables, employeeId);
        const totalRows = Object.values(tables).reduce((sum, arr) => sum + arr.length, 0);
        Logger_1.default.log(`[DataSyncService] Delta export (since ${new Date(sinceTs).toISOString()}): ${Object.keys(tables).length} tables, ${totalRows} rows`);
     ...
     */
    importFullSync(payload, zaloIds, onProgress) {
        const db = DatabaseService_1.default.getInstance();
        ...
        for (const acc of payload.accounts || []) {
            try {
                ...
                Logger_1.default.warn(`[DataSyncService] Import account error: ${err.message}`);
            }
        }
        // Import tables
        ...
        Logger_1.default.log(`[DataSyncService] Full import complete: ${tableNames.length} tables`);
    }
    /**
     * Import a delta sync payload: upsert changed rows.
     */
    importDeltaSync(payload, onProgress) {
        const db = DatabaseService_1.default.getInstance();
        ...
        const tableNames = Object.keys(payload.tables || {});
        for (const tableName of tableNames) {
            if (!PRIVACY_FILTERED_ERP_TABLES.has(tableName))
                continue;
            try {
        ...
        Logger_1.default.log(`[DataSyncService] Delta import complete: ${tableNames.length} tables`);
    }
    /**
     * Reset local employee DB: delete all synced data.
     */
    resetEmployeeDB(zaloIds) {
        const db = DatabaseService_1.default.getInstance();
        const inClause = zaloIds.map(id => `'${this.esc(id)}'`).join(',');
        for (const spec of SYNCABLE_TABLES_BY_ZALO) {
            try {
        ...
        db.forceFlush();
        Logger_1.default.log(`[DataSyncService] Employee DB reset for ${zaloIds.length} accounts`);
    }
    appendPrivateErpTables(tables, employeeId) {
        const db = DatabaseService_1.default.getInstance();
        ...
        if (calendarEvents.length > 0) {
            const eventIds = calendarEvents.map((row) => row.id);
        ...
        if (ownedFolders.length > 0)
            tables.erp_note_folders = ownedFolders;
        if (notes.length > 0) {
            ...
            const tagIds = Array.from(new Set((tables.erp_note_tag_map || []).map((row) => row.tag_id).filter((id) => id !== null && id !== undefined)));
            if (tagIds.length > 0) {
                tables.erp_note_tags = this.queryByIds(db, 'erp_note_tags', 'id', tagIds);
            }
        }
    }
    getAccessibleCalendarEvents(db, employeeId) {
    ...
    getOwnedNoteFolders(db, employeeId) {
        return db.query('SELECT * FROM erp_note_folders WHERE owner_id = ?', [employeeId]);
    }
    getAccessibleNotes(db, employeeId) {
        const params = [employeeId, employeeId];
        let sql = `
            SELECT DISTINCT n.*
            FROM erp_notes n
    ...
    queryByIds(db, tableName, column, ids, sinceTs) {
    ...
    bulkInsert(db, tableName, rows) {
        ...
        try {
            ...
            Logger_1.default.warn(`[DataSyncService] bulkInsert ${tableName} transaction error: ${err.message}`);
        }
        if (inserted > 0) {
            Logger_1.default.info(`[DataSyncService] Inserted ${inserted}/${rows.length} rows into ${tableName}`);
        }
    }
