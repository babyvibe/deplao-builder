...
class ErpTaskService {
    static getInstance() {
        if (!this.instance)
            this.instance = new ErpTaskService();
    ...
    listProjects(filter) {
        ...
        return this.db().query(`SELECT * FROM erp_projects WHERE status = ? ORDER BY created_at ASC`, [status]);
    }
    getProject(id) {
        return this.db().queryOne(`SELECT * FROM erp_projects WHERE id = ?`, [id]);
    }
    createProject(input, employeeId) {
        const id = (0, uuid_1.v4)();
        const now = Date.now();
        this.db().run(`INSERT INTO erp_projects (id, name, description, color, owner_employee_id, department_id, status, created_at, updated_at)
       VALUES (?, ?, ?, ?, ?, ?, 'active', ?, ?)`, [id, input.name, input.description ?? '', input.color ?? '#3b82f6', employeeId, input.department_id ?? null, now, now]);
        const project = this.getProject(id);
        EventBroadcaster_1.default.emit('erp:event:projectCreated', { project });
        return project;
    }
    updateProject(id, patch) {
        ...
        const project = this.getProject(id);
        EventBroadcaster_1.default.emit('erp:event:projectUpdated', { project });
        return project;
    }
    deleteProject(id) {
        this.db().transaction(() => {
        ...
        EventBroadcaster_1.default.emit('erp:event:projectDeleted', { projectId: id });
    }
    // ─── Tasks ─────────────────────────────────────────────────────────────────
    listTasks(filter = {}) {
        let sql = `
      ...
      WHERE t.archived = ?
    `;
        const params = [filter.archived ? 1 : 0];
        if (filter.projectId) {
    ...
    `, [id]);
        if (!task)
            return undefined;
        const checklist = this.db().query(`SELECT * FROM erp_task_checklist WHERE task_id = ? ORDER BY sort_order ASC`, [id]);
        const comments = this.db().query(`SELECT * FROM erp_task_comments WHERE task_id = ? ORDER BY created_at ASC`, [id]).map(c => ({ ...c, mentions: this._safeJsonArray(c.mentions) }));
        const attachments = this.db().query(`SELECT * FROM erp_task_attachments WHERE task_id = ? ORDER BY uploaded_at ASC`, [id]);
        const activity = this.db().query(`SELECT * FROM erp_task_activity_log WHERE task_id = ? ORDER BY created_at DESC LIMIT 50`, [id]).map(a => ({ ...a, payload: this._safeJsonObject(a.payload) }));
        const watchers = this.db().query(`SELECT employee_id FROM erp_task_watchers WHERE task_id = ?`, [id]).map(r => r.employee_id);
        const dependencies = this.db().query(`SELECT * FROM erp_task_dependencies WHERE task_id = ?`, [id]);
        return {
            ...task,
    ...
    // ─── Watchers (Phase 2) ───────────────────────────────────────────────────
    addWatcher(taskId, employeeId) {
        this.db().run(`INSERT OR IGNORE INTO erp_task_watchers (task_id, employee_id, added_at) VALUES (?,?,?)`, [taskId, employeeId, Date.now()]);
        const task = this._getTaskLite(taskId);
        EventBroadcaster_1.default.emit('erp:event:taskUpdated', { taskId, patch: { watchers: 'added' }, task });
    }
    removeWatcher(taskId, employeeId) {
        this.db().run(`DELETE FROM erp_task_watchers WHERE task_id = ? AND employee_id = ?`, [taskId, employeeId]);
        const task = this._getTaskLite(taskId);
        EventBroadcaster_1.default.emit('erp:event:taskUpdated', { taskId, patch: { watchers: 'removed' }, task });
    }
    // ─── Dependencies (Phase 2) ───────────────────────────────────────────────
    addDependency(taskId, dependsOnId, type = 'FS') {
        if (taskId === dependsOnId)
            throw new Error('Task không thể phụ thuộc chính nó');
        ...
        while (queue.length) {
            ...
            if (cur === taskId)
                throw new Error('Phụ thuộc tạo vòng lặp');
            if (visited.has(cur))
                continue;
            visited.add(cur);
            const next = this.db().query(`SELECT depends_on_task_id FROM erp_task_dependencies WHERE task_id = ?`, [cur]);
            for (const r of next)
                queue.push(r.depends_on_task_id);
        }
        this.db().run(`INSERT OR IGNORE INTO erp_task_dependencies (task_id, depends_on_task_id, type, created_at) VALUES (?,?,?,?)`, [taskId, dependsOnId, type, Date.now()]);
        const task = this._getTaskLite(taskId);
        EventBroadcaster_1.default.emit('erp:event:taskUpdated', { taskId, patch: { dependency: 'added' }, task });
    }
    removeDependency(taskId, dependsOnId) {
        this.db().run(`DELETE FROM erp_task_dependencies WHERE task_id = ? AND depends_on_task_id = ?`, [taskId, dependsOnId]);
        const task = this._getTaskLite(taskId);
        EventBroadcaster_1.default.emit('erp:event:taskUpdated', { taskId, patch: { dependency: 'removed' }, task });
    }
    /** Throws if task has any incomplete blocker. */
    _assertDependenciesSatisfied(taskId) {
        const blockers = this.db().query(`SELECT d.depends_on_task_id, t.status, t.title
       FROM erp_task_dependencies d
       JOIN erp_tasks t ON t.id = d.depends_on_task_id
       WHERE d.task_id = ? AND t.status NOT IN ('done','cancelled')`, [taskId]);
        if (blockers.length) {
            const titles = blockers.map(b => b.title).slice(0, 3).join(', ');
            throw new Error(`Task bị chặn bởi: ${titles}${blockers.length > 3 ? '…' : ''}`);
        }
    }
    createTask(input, reporterId) {
        const id = (0, uuid_1.v4)();
        const now = Date.now();
        this.db().transaction(() => {
            this.db().run(`INSERT INTO erp_tasks
        ...
        const task = this._getTaskLite(id) ?? { id };
        EventBroadcaster_1.default.emit('erp:event:taskCreated', { task });
        ...
        if (input.assignees?.length) {
            for (const empId of input.assignees) {
                if (empId && empId !== reporterId) {
                    try {
                        ErpNotificationService_1.default.getInstance().notify(empId, 'task_assigned', `Bạn được giao task: ${input.title}`, `Được giao bởi ${reporterId}`, `erp://task/${id}`);
                    }
                    catch (err) {
                        Logger_1.default.warn(`[ErpTaskService] notify assignee ${empId} failed: ${err.message}`);
                    }
                }
            }
        }
        ...
        if (patch.due_date !== undefined) {
            try {
                ...
                if (patch.due_date && typeof patch.due_date === 'number') {
                    ErpCalendarService.getInstance().syncTaskToCalendar(id, task.title ?? '(Task)', patch.due_date, actorId);
                }
            }
            catch (err) {
                Logger_1.default.warn(`[ErpTaskService] syncTaskToCalendar skipped: ${err.message}`);
            }
        }
        EventBroadcaster_1.default.emit('erp:event:taskUpdated', { taskId: id, patch, actorId, task });
        return task;
    }
    assignTask(id, employeeIds, actorId) {
        const now = Date.now();
        this.db().transaction(() => {
            this.db().run(`DELETE FROM erp_task_assignees WHERE task_id = ?`, [id]);
        ...
        EventBroadcaster_1.default.emit('erp:event:taskUpdated', { taskId: id, patch: { assignees: employeeIds }, actorId, task });
    }
    deleteTask(id) {
        const existingAttachments = this.db().query(`SELECT file_path FROM erp_task_attachments WHERE task_id = ?`, [id]);
        this.db().transaction(() => {
            this.db().run(`DELETE FROM erp_task_assignees WHERE task_id = ?`, [id]);
        ...
        for (const attachment of existingAttachments) {
            FileStorageService_1.default.deleteManagedTaskAttachment(attachment.file_path);
        }
        EventBroadcaster_1.default.emit('erp:event:taskDeleted', { taskId: id });
    }
    // ─── Checklist ─────────────────────────────────────────────────────────────
    addChecklist(taskId, content) {
        const now = Date.now();
        const sortOrderRow = this.db().queryOne(`SELECT MAX(sort_order) as m FROM erp_task_checklist WHERE task_id = ?`, [taskId]);
        const sortOrder = (sortOrderRow?.m ?? 0) + 1;
        const newId = this.db().runInsert(`INSERT INTO erp_task_checklist (task_id, content, done, sort_order, created_at) VALUES (?,?,0,?,?)`, [taskId, content, sortOrder, now]);
        const item = this.db().queryOne(`SELECT * FROM erp_task_checklist WHERE id = ?`, [newId]);
        this._touchTask(taskId);
        return item;
    }
    toggleChecklist(id, done) {
        this.db().run(`UPDATE erp_task_checklist SET done = ? WHERE id = ?`, [done ? 1 : 0, id]);
        return this.db().queryOne(`SELECT * FROM erp_task_checklist WHERE id = ?`, [id]);
    }
    // ─── Comments ──────────────────────────────────────────────────────────────
    addComment(taskId, authorId, content, mentions = []) {
        const now = Date.now();
        const newId = this.db().runInsert(`INSERT INTO erp_task_comments (task_id, author_id, content, mentions, created_at, updated_at) VALUES (?,?,?,?,?,?)`, [taskId, authorId, content, JSON.stringify(mentions), now, now]);
        const comment = this.db().queryOne(`SELECT * FROM erp_task_comments WHERE id = ?`, [newId]);
        this._logActivity(taskId, authorId, 'commented', { preview: content.slice(0, 50) });
        this._touchTask(taskId);
        const task = this._getTaskLite(taskId);
        EventBroadcaster_1.default.emit('erp:event:commentAdded', { taskId, comment: { ...comment, mentions }, task });
        EventBroadcaster_1.default.emit('erp:event:taskUpdated', { taskId, patch: { comment_count: task?.comment_count || 0 }, task });
        return { ...comment, mentions };
    }
    editComment(id, content) {
        const now = Date.now();
        this.db().run(`UPDATE erp_task_comments SET content = ?, updated_at = ? WHERE id = ?`, [content, now, id]);
        const comment = this.db().queryOne(`SELECT * FROM erp_task_comments WHERE id = ?`, [id]);
    ...
    // ─── Private helpers ───────────────────────────────────────────────────────
    _assignTask(taskId, employeeIds, now) {
        for (const empId of employeeIds) {
            if (!empId)
                continue;
            this.db().run(`INSERT OR IGNORE INTO erp_task_assignees (task_id, employee_id, assigned_at) VALUES (?,?,?)`, [taskId, empId, now]);
        }
    }
    _replaceWatchers(taskId, employeeIds, now) {
        ...
        for (const empId of uniqueIds) {
            this.db().run(`INSERT OR IGNORE INTO erp_task_watchers (task_id, employee_id, added_at) VALUES (?,?,?)`, [taskId, empId, now]);
        }
    }
    _replaceAttachments(taskId, attachments, actorId, now) {
        const existingAttachments = this.db().query(`SELECT file_path FROM erp_task_attachments WHERE task_id = ?`, [taskId]);
        ...
        for (const item of normalized) {
            const stored = FileStorageService_1.default.saveTaskAttachment(taskId, item.file_path, item.file_name);
            if (!stored.filePath)
                continue;
            keptPaths.add(stored.filePath);
            this.db().run(`INSERT INTO erp_task_attachments (task_id, file_name, file_path, mime_type, size, uploaded_by, uploaded_at) VALUES (?,?,?,?,?,?,?)`, [taskId, stored.fileName, stored.filePath, item.mime_type, item.size || stored.size, actorId, now]);
        }
        for (const attachment of existingAttachments) {
    ...
    _logActivity(taskId, actorId, action, payload) {
        this.db().run(`INSERT INTO erp_task_activity_log (task_id, actor_id, action, payload, created_at) VALUES (?,?,?,?,?)`, [taskId, actorId, action, JSON.stringify(payload), Date.now()]);
    }
    _touchTask(taskId) {
...
exports.default = ErpTaskService;
//# sourceMappingURL=ErpTaskService.js.map