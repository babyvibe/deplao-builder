 ...
 */
class GHNAdapter extends IntegrationAdapter_1.IntegrationAdapter {
    constructor() {
        super(...arguments);
        this.type = 'ghn';
        this.name = 'GHN Express';
    }
    getTokenHeaders() {
        return {
    ...
    async testConnection() {
        try {
            const res = await axios_1.default.get(`${this.getBaseUrl()}/v2/shop/all`, {
    ...
    async executeAction(action, params) {
        ...
        switch (action) {
            case 'createOrder': {
                const res = await axios_1.default.post(`${base}/v2/shipping-order/create`, params.order || params, {
                    headers,
                    timeout: 15000,
                });
            ...
            case 'getTracking': {
                const res = await axios_1.default.post(`${base}/v2/shipping-order/detail`, { order_code: params.orderCode }, { headers, timeout: 10000 });
                return { tracking: res.data?.data || {} };
            }
            case 'cancelOrder': {
                const res = await axios_1.default.post(`${base}/v2/switch-status/cancel`, { order_codes: [params.orderCode] }, { headers, timeout: 10000 });
                return { success: true, data: res.data?.data };
            }
            case 'calculateFee': {
                const res = await axios_1.default.post(`${base}/v2/shipping-order/fee`, params, {
                    headers,
                    timeout: 10000,
                });
            ...
            case 'getProvinces': {
                const res = await axios_1.default.get(`${base}/master-data/province`, {
                    headers: { Token: this.config.credentials.token },
                    timeout: 10000,
                });
            ...
            case 'getDistricts': {
                const res = await axios_1.default.get(`${base}/master-data/district`, {
            ...
            case 'getWards': {
                ...
                if (!districtId)
                    throw new Error('Thiếu districtId để lấy phường/xã GHN');
                const res = await axios_1.default.post(`${base}/master-data/ward`, { district_id: districtId }, {
                    headers: this.getTokenHeaders(),
                    timeout: 10000,
                });
            ...
            case 'getServices': {
                ...
                const res = await axios_1.default.post(`${base}/v2/shipping-order/available-services`, {
                    shop_id: shopId,
...
exports.GHNAdapter = GHNAdapter;
//# sourceMappingURL=GHNAdapter.js.map