...
exports.strBase = strBase;
exports.parseCookieString = parseCookieString;
exports.getFacebookIDFromCookie = getFacebookIDFromCookie;
exports.getNextReqId = getNextReqId;
exports.generateSessionId = generateSessionId;
exports.generateClientId = generateClientId;
exports.randStr = randStr;
exports.genThreadingId = genThreadingId;
exports.buildHeaders = buildHeaders;
exports.buildFormData = buildFormData;
exports.jsonMinimal = jsonMinimal;
exports.dataSplit = dataSplit;
exports.parseFBResponse = parseFBResponse;
exports.buildPostConfig = buildPostConfig;
...
function strBase(number, base) {
    if (number < 0)
        return '-' + strBase(-number, base);
    const d = Math.floor(number / base);
    const m = number % base;
    if (d > 0)
        return strBase(d, base) + digitToChar(m);
 ...
 */
function getFacebookIDFromCookie(cookie) {
 ...
 */
function buildHeaders(cookie, contentLength, host = 'www.facebook.com') {
    const headers = {
        'Host': host,
 ...
 */
function buildFormData(dataFB, opts) {
 ...
 */
function dataSplit(html, before, after) {
    const startIdx = html.indexOf(before);
    if (startIdx === -1)
        throw new Error(`dataSplit: "${before}" not found`);
    const valueStart = startIdx + before.length;
    const endIdx = html.indexOf(after, valueStart);
    if (endIdx === -1)
        throw new Error(`dataSplit: closing "${after}" not found`);
 ...
 */
function buildPostConfig(url, formData, cookie, host = 'www.facebook.com') {
 ...
 */
function buildGetConfig(url, cookie, host = 'www.facebook.com') {
    return {
        url,
...
//# sourceMappingURL=FacebookUtils.js.map