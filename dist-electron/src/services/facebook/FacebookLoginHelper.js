...
function generateDeviceId() {
    return `${(0, FacebookUtils_1.randStr)(8)}-${(0, FacebookUtils_1.randStr)(4)}-${(0, FacebookUtils_1.randStr)(4)}-${(0, FacebookUtils_1.randStr)(4)}-${(0, FacebookUtils_1.randStr)(12)}`;
}
function buildCookieExport(sessionCookies) {
    return (sessionCookies || [])
        .filter((c) => c?.name && c?.value)
        .map((c) => `${c.name}=${c.value}; `)
        .join('');
}
function buildLoginResult(dataJson, statusLogin, cookies) {
    if (statusLogin === 1) {
        return {
            success: {
...
async function postLogin(data) {
    try {
        ...
        const response = await axios_1.default.post(FB_AUTH_URL, body, {
...
async function getToken2FA(key2FA) {
    try {
        if (!key2FA)
            return '';
        ...
        Logger_1.default.warn(`[FacebookLoginHelper] 2FA token error: ${err.message}`);
 ...
 */
async function loginWithCredentials(username, password, twoFASecret) {
    const deviceId = generateDeviceId();
    const machineId = (0, FacebookUtils_1.randStr)(24);
    const adId = deviceId;
    const baseForm = (pw, credentialsType, tryNum) => {
        const form = {
            adid: adId,
            format: 'json',
            device_id: deviceId,
